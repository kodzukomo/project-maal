...
import { ArrowLeft, Thermometer, Cpu, ShieldCheck, MapPin, ClipboardList } from "lucide-react";
import TelemetryCard from "@/components/TelemetryCard";
import RemoteControl from "@/components/RemoteControl";
import UnitAlerts from "@/components/UnitAlerts";

const statusConfig = {
  operational: { label: "Operational", dot: "bg-blue-400", text: "text-blue-400", badge: "bg-blue-500/10 border-blue-500/20" },
  warning: { label: "Warning", dot: "bg-slate-400", text: "text-slate-400", badge: "bg-slate-500/10 border-slate-500/20" },
  critical: { label: "Critical", dot: "bg-red-400", text: "text-red-400", badge: "bg-red-500/10 border-red-500/20" },
...
export default function UnitDetail() {
  ...
  const sendCommand = async (cmd) => {
    const updates = {
      standby: { status: "warning", current_task: "Standby Mode — tasks paused", last_command: cmd.label },
      return_base: { current_task: "Returning to base", last_command: cmd.label },
      diagnostics: { current_task: "Running hardware diagnostics", last_command: cmd.label },
      shutdown: { status: "offline", current_task: "Emergency shutdown executed", last_command: cmd.label },
    };
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6 max-w-[1400px] mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        <TelemetryCard icon={Thermometer} label="Temperature" value={android.temperature} unit="°C" status={tempStatus} />
        <TelemetryCard icon={Cpu} label="CPU Load" value={android.cpu_load} unit="%" status={cpuStatus} />
        <TelemetryCard icon={ShieldCheck} label="Hardware Integrity" value={android.hardware_integrity} unit="%" status={hwStatus} />
      </div>
        ...
        <div className="space-y-6">
          <div className="rounded-xl border border-border bg-card/40 p-5">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="text-sm font-display font-semibold text-foreground">Location</h3>
            </div>
          ...
          <div className="rounded-xl border border-border bg-card/40 p-5">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="text-sm font-display font-semibold text-foreground">Current Task</h3>
            </div>
